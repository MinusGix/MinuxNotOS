...
namespace ISR {
    ...
    template<typename T>
    bool setInterruptHandler (uint32_t int_number, T func, bool force=false) {
        if (force || !interrupt_handlers[int_number].hasFunction()) {
            interrupt_handlers[int_number] = func;
            return true;
        }
        return false;
    }
